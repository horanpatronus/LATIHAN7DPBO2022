...
if(isset($_POST['add'])){
	...
	$otask->insertTask($name, $details, $subject, $priority, $deadline);
...
if(isset($_GET['id_delete'])){
	$otask->deleteTask($_GET['id_delete']);

	header("location:index.php");
}

if(isset($_GET['id_sorting'])){
	$otask->sortingTask($_GET['id_sorting']);
}
...
while (list($id, $tname, $tdetails, $tsubject, $tpriority, $tdeadline, $tstatus) = $otask->getResult()) {
	...
	if($tstatus == "Sudah"){
		...
		<td>" . $tstatus . "</td>
		<td>
		<button class='btn btn-danger'><a href='index.php?id_delete=" . $id . "' style='color: white; font-weight: bold;'>Hapus</a></button>
		</td>
		</tr>";
		$no++;
	}
